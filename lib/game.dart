...
class _LudoGameState extends State<LudoGame> with TickerProviderStateMixin {
  ...
  final List<List<MapEntry<int, Rect>>> _pawnCurrentStepInfo = []; //step index, rect
  ...
  @override
  void dispose() {
    for (List<AnimationController> controllerList in _playerAnimContList) {
      for (AnimationController controller in controllerList) {
        controller.dispose();
      }
    }
  ...
  bool _checkCollision(MapEntry<int, Rect> currentStepInfo) {
    ...
    if (currentStepInfo.key < 52) { //no need to check if the pawn has entered destination lane: không cần kiểm tra xem quân tốt đã vào làn đích chưa

      if (!_safeSpots!.any((safeSpot) => safeSpot.contains(currentStepCenter))) { //avoid checking if it has landed on a safe spot: tránh kiểm tra xem nó đã hạ cánh ở nơi an toàn chưa
         ...
         */
        if (collisions.isEmpty || collisions.any((collision) => collision.targetPlayerIndex == _currentTurn) || collisions.length > 1) {
          _collisionDetails.isReverse = false;
        } else {
          _collisionDetails = collisions.first;
          _playerAnimContList[_collisionDetails.targetPlayerIndex!][_collisionDetails.pawnIndex!].duration = Duration(milliseconds: _reverseStepAnimTimeInMillis);
  ...
  _changeTurn() {
    if (_winnerPawnList.where((playerPawns) => playerPawns.length == 4).length != 3) { //if any 3 players have completed
      _highlightDice();
      ...
      if (!_provideFreeTurn) {
        do {
          //to ignore winners
          _currentTurn = (_currentTurn + 1) % 4; //change turn after animation completes
          if (_winnerPawnList[_currentTurn].length != 4) {
